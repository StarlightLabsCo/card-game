...
namespace Starlight.BattleMembers
{
    ...
    public class PlayerBehaviour : BattleMember
    {
	   ...
	   private void ForgeWords()
	   {
		  ...
		  //create new card
		  WordData[] words = hammerMenuWords.ConvertAll(x => x.Data).ToArray();
		  CardData newCard = new(words);

		  //clear hammer menu
		  hammerMenuWords.ForEach(word => Destroy(word.gameObject));
		  hammerMenuWords.Clear();

		  var cardUI = CardManager.instance.CreateCard(cardParent, newCard);
		  cardUI.SetBig();
	   }
